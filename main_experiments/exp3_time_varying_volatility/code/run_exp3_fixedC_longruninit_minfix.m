...
% Formal fixed-C Experiment 3 with stage-0 marginal variance at the GARCH
% long-run level. This is the single entry point for all reported Exp3
% tables and figures. The true-population oracle/regret code below uses no
% ridge, eigenvalue clipping, evaluation moments, or inverse.
...
experiment_dir = fileparts(code_dir);
results_dir = fullfile(experiment_dir, 'results');
figures_dir = fullfile(experiment_dir, 'figures');
audit_dir = fullfile(experiment_dir, 'audit');
...
config.selection_provenance = ['Current paper specification: stage-0 volatility is ' ...
    'sqrt(omega/(1-alpha-beta)), without the 0.005 floor; the 0.005 floor applies ' ...
    'from stage 1 onward. The fixed numeric C was selected from replication 500 ' ...
    'of the preceding screening run and is held fixed here.'];
...
if ~isempty(opts.FixedCorrelationMatrix)
    ...
    config.selection_provenance = sprintf(['Runtime fixed-C override from source replication %d. ' ...
        'This override is intended for C screening or a prespecified formal fixed-C rerun.'], ...
...
% The published coefficient bands and bootstrap-summary table use a
% separate, fixed reporting stream. This does not alter simulation seeds.
config.reporting_bootstrap_repetitions = 2000;
config.reporting_bootstrap_seed = 20260913;
...
if strcmp(mode,'finalize')
    assert(exist(partial_raw_file,'file')==2,'Exp3Formal:MissingCompletedPartialRun', ...
        'The completed full-run partial MAT is required for finalize mode.');
    assert(exist(raw_file,'file')~=2,'Exp3Formal:ProtectedOutputExists', ...
...
try
    source_audit = verify_executed_sources(code_dir, config);
    write_parameter_audit(fullfile(audit_dir, ...
        'exp3_fixedC_longruninit_minfix_PARAMETER_AUDIT.csv'),config,source_audit);
    addpath(code_dir);
    cleanup_path = onCleanup(@() rmpath(code_dir)); %#ok<NASGU>
    assert(exist('run_eto','file') == 2 && exist('run_ieo','file') == 2, ...
        'Exp3Formal:MissingStrategyCode', 'Packaged strategy functions not found.');
    assert(exist('quadprog','file') == 2, 'Exp3Formal:MissingQuadprog', ...
    ...
    if strcmp(mode,'finalize')
        ...
        simulation = execute_formal_run(config, partial_raw_file, opts.Verbose);
    end
    checks = validate_formal_run(simulation, config);
    ...
    if ~checks.pass
        error('Exp3Formal:ValidationFailure', '%s', strjoin(checks.failures, newline));
    end
    ...
    create_formal_figures(simulation, summaries, config, figures_dir, opts.ShowFigures);
    ...
    fprintf('Full formal Exp3 run passed and all outputs were written.\n');
    fprintf('Finished: %s\n', char(datetime('now','Format','yyyy-MM-dd HH:mm:ss Z')));
    diary off;
catch ME
    diary off;
    if strcmp(ME.identifier, 'Exp3Formal:TrueSigma1NotSPD')
    ...
    remove_formal_figures(figures_dir);
    rethrow(ME);
end
end
...
function simulation = execute_formal_run(config, partial_raw_file, verbose)
...
for s = 1:S
    ...
    if verbose && (s == 1 || mod(s,25)==0 || s==S)
        fprintf('Exp3 replication %d/%d complete (seed %d).\n',s,S,current_seed);
    end
end
...
for t=T:-1:1
    ...
    if eig_min < -tol_spd
        error('Exp3Formal:TrueSigma1NotSPD', ...
    ...
    if relative_difference(u_formula,u_kkt)>config.crosscheck_relative_tolerance
        error('Exp3Formal:OracleFormulaMismatch', ...
    ...
    if d.alternative_crosscheck_flag(t)
        ...
        if ~pass
            error('Exp3Formal:SolverCrosscheckFailure', ...
...
if isfield(config,'fixed_correlation_matrix') && ~isempty(config.fixed_correlation_matrix)
    ...
    Corr=(Corr+Corr')/2;
    assert(max(abs(diag(Corr)-1))<1e-10,'Exp3Formal:FixedCBadDiagonal', ...
        'Fixed correlation matrix does not have unit diagonal.');
    assert(min(eig(Corr))>0,'Exp3Formal:FixedCNotSPD', ...
        'Fixed correlation matrix is not positive definite.');
else
    Craw=0.2+0.6*rand(n); Craw=(Craw+Craw')/2; Craw(1:n+1:end)=1;
    % Fixed formal DGP: floor eigenvalues only. This is not clipping of
...
for t=1:T
    ...
    if flag~=0, error('Exp3Formal:DGPFailure','Exact DGP covariance was not PD.'); end
...
function checks=validate_formal_run(sim,config)
...
[bootstrap_tbl,coefficient_ci]=pointwise_bootstrap_summary(sim,config);
bootstrap_file=fullfile(results_dir,'exp3_fixedC_longruninit_minfix_bootstrap_summary.csv');
writetable(bootstrap_tbl,bootstrap_file);
...
ctbl=table(stages,mean(sim.slope_gap_sims,1)',mean(sim.intercept_gap_sims,1)', ...
    ...
    coefficient_ci.slope_gap_sims(1,:)',coefficient_ci.slope_gap_sims(2,:)', ...
    coefficient_ci.intercept_gap_sims(1,:)',coefficient_ci.intercept_gap_sims(2,:)', ...
    coefficient_ci.slope_angle_sims(1,:)',coefficient_ci.slope_angle_sims(2,:)', ...
    coefficient_ci.intercept_angle_sims(1,:)',coefficient_ci.intercept_angle_sims(2,:)', ...
    ...
    'slope_gap_bootstrap_ci_low','slope_gap_bootstrap_ci_high', ...
    'intercept_gap_bootstrap_ci_low','intercept_gap_bootstrap_ci_high', ...
    'slope_angle_bootstrap_ci_low','slope_angle_bootstrap_ci_high', ...
    'intercept_angle_bootstrap_ci_low','intercept_angle_bootstrap_ci_high'});
...
summaries=struct('stage',tbl,'wealth',wtbl,'coefficient',ctbl,'near',near_tbl, ...
    'bootstrap',bootstrap_tbl,'high',high_tbl,'extreme',extreme_tbl,'stage_file',stage_file, ...
    'wealth_file',wealth_file,'coefficient_file',coef_file,'near_file',near_file, ...
    'bootstrap_file',bootstrap_file,'high_file',high_file,'extreme_file',extreme_file,'config_file',config_file);
end
...
function [tbl,ci_by_field]=pointwise_bootstrap_summary(sim,config)
% Pointwise percentile-bootstrap CIs for Monte Carlo means. Each statistic
% restarts the same documented reporting stream, so every row is exactly
% traceable to its seed and number of resamples. These are not simultaneous
% bands and are not mean +/- 1.96 SE intervals.
names={'Regret_ETO_pop_sims','Regret_IEO_pop_sims','Wealth_Paths_ETO', ...
    'Wealth_Paths_IEO','intercept_angle_sims','intercept_gap_sims', ...
    'slope_angle_sims','slope_gap_sims'};
total=sum(cellfun(@(name)size(sim.(name),2),names));
statistic_name=strings(total,1); stage_t=zeros(total,1); mean_value=zeros(total,1);
ci_low=zeros(total,1); ci_high=zeros(total,1); row=0; ci_by_field=struct();
for k=1:numel(names)
    name=names{k}; X=sim.(name);
    [lo,hi]=pointwise_bootstrap_mean_ci(X,config.reporting_bootstrap_repetitions, ...
        config.reporting_bootstrap_seed);
    ci_by_field.(name)=[lo;hi]; nstage=size(X,2); ids=row+(1:nstage);
    statistic_name(ids)=string(name); stage_t(ids)=(0:nstage-1)';
    mean_value(ids)=mean(X,1,'omitnan')'; ci_low(ids)=lo'; ci_high(ids)=hi';
    row=row+nstage;
end
bootstrap_seed=repmat(config.reporting_bootstrap_seed,total,1);
B_boot=repmat(config.reporting_bootstrap_repetitions,total,1);
tbl=table(statistic_name,stage_t,mean_value,ci_low,ci_high,bootstrap_seed,B_boot, ...
    'VariableNames',{'statistic_name','stage_t','mean','bootstrap_ci_low', ...
    'bootstrap_ci_high','bootstrap_seed','B_boot'});
end

function [lo,hi]=pointwise_bootstrap_mean_ci(X,B,seed)
state=rng; cleanup=onCleanup(@()rng(state)); %#ok<NASGU>
rng(seed,'twister'); N=size(X,1); T=size(X,2); boot=nan(B,T);
for first=1:100:B
    ids=first:min(first+99,B); ix=randi(N,N,numel(ids));
    for t=1:T
        y=X(:,t); boot(ids,t)=mean(y(ix),1,'omitnan')';
    end
end
q=prctile(boot,[2.5,97.5],1); lo=q(1,:); hi=q(2,:);
end
...
function create_formal_figures(sim,sumry,config,figures_dir,show_figures)
...
if any(me<=0 | mi<=0)
    error('Exp3Formal:LogFigureFailure','Nonpositive mean prevents an unaltered semilogy plot.');
end
f=figure('Visible',vis,'Color','w','Position',[100 100 700 460]);
...
mean_fields={'mean_intercept_gap','mean_slope_gap','mean_intercept_angle_degrees','mean_slope_angle_degrees'};
low_fields={'intercept_gap_bootstrap_ci_low','slope_gap_bootstrap_ci_low', ...
    'intercept_angle_bootstrap_ci_low','slope_angle_bootstrap_ci_low'};
high_fields={'intercept_gap_bootstrap_ci_high','slope_gap_bootstrap_ci_high', ...
    'intercept_angle_bootstrap_ci_high','slope_angle_bootstrap_ci_high'};
ylabs={'Intercept gap','Slope gap','Intercept angle (degrees)','Slope angle (degrees)'};
for k=1:numel(names)
    mu=sumry.coefficient.(mean_fields{k})';
    clo=sumry.coefficient.(low_fields{k})'; chi=sumry.coefficient.(high_fields{k})';
    f=figure('Visible',vis,'Color','w','Position',[100 100 700 460]);
    patch([t fliplr(t)],[clo fliplr(chi)],blue,'FaceAlpha',0.18, ...
        'EdgeColor','none','DisplayName','Pointwise 95% percentile-bootstrap CI'); hold on;
    plot(t,mu,'-','Color',blue,'LineWidth',2.0,'DisplayName','Monte Carlo mean');
    format_axes('Stage t',ylabs{k}); legend('Location','best');
    export_pair(f,figures_dir,['exp3_' names{k}]); close(f);
end
end
...
fprintf(fid,'This leave-one-out calculation is **NOT FOR PAPER MAIN RESULT**. No observation was removed from any formal output.\n\n');
...
fprintf(fid,'\nThe formal curves retain replication 875. Reliability is judged from the recorded KKT, budget, p/q residual and solver-cross-check diagnostics.\n');
end
...
fprintf(fid,'1. Current Exp3 runner and executed helper hashes recorded before execution: YES.\n');
fprintf(fid,'2. Outputs are written only under this Exp3 package: YES.\n3. This is the single formal Exp3 entry point: YES.\n');
fprintf(fid,'\n## B. DGP\n\n1. Fixed numeric C source replication=%d; eigenvalue-floor provenance %.17g.\n2. True moments are replication-specific through volatility and epsilon_mu paths, while C is fixed across replications and stages.\n3. Cross-replication moment averaging: NO.\n',config.fixed_C_source_replication,config.correlation_eigenvalue_floor);
...
fprintf(fid,'\nImportant scope note: the run_eto/run_ieo estimation helpers retain their documented estimation regularization/fallbacks. The no-ridge statement above applies to the true population oracle, Sigma1, and population regret.\n');
...
fprintf(fid,'The S=%d same-run formal experiment passed dry/full validation. Overall mean population regret is %.17g (ETO) and %.17g (IEO).\n',config.S,overall_e,overall_i);
fprintf(fid,'This is the current formal Experiment 3 reproduction run. The fixed C was selected previously rather than in an independent prespecified confirmation. Selection provenance: %s\n',config.selection_provenance);
fprintf(fid,'\n## 2. Same-run construction\n\nTrue paths, training/evaluation samples, learned policies, population oracle/regret, wealth and coefficient diagnostics were generated together replication-by-replication and saved in `%s`.\n',report_path(raw_file));
...
fprintf(fid,'Source hashes: current runner `%s`; generate_data `%s`.\n',source.main_sha256,source.generate_data_sha256);
fprintf(fid,'\n## 4. Policy estimation verification\n\nThe packaged run_eto (`%s`) and run_ieo (`%s`) helpers were called. Full A/B arrays were saved; u(x)=A*x+B.\n',source.run_eto_sha256,source.run_ieo_sha256);
...
fprintf(fid,'\n## 10. Extreme point analysis\n\nTop 20 global points plus every >10%% stage-mean contributor are in `%s`. No replication was deleted, winsorized or smoothed.\n',report_path(sumry.extreme_file));
fprintf(fid,'\n## 11. Same-run wealth results\n\nSame-run ETO/IEO wealth is in `%s` and in both two-panel figures.\n',report_path(sumry.wealth_file));
fprintf(fid,'\n## 12. Metric scope\n\nEvery reported regret output is true-population regret. No evaluation-sample proxy is computed or imported.\n');
fprintf(fid,'\n## 13. Reporting note\n\nPresent raw mean and Delta_R_pop figures together. Because 3e-5 was selected after outcome-directed screening, this formal S=2000 run must not be described as independent confirmatory evidence; the DGP selection provenance must be disclosed.\n');
fprintf(fid,'\n## 14. Remaining risks\n\nRaw means may be dominated by valid extreme paths. The ETO/IEO estimation helpers retain their documented numerical regularization; this is not introduced into the population oracle/regret. Near-singular true matrices make oracle coefficients large even when KKT and budget residuals pass.\n');
...
fid=fopen(path,'w'); c=onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid,'# Experiment 3 formal population-regret dry run: PASS\n\n');
...
fprintf(fid,'- Executed source hashes run_eto/run_ieo: `%s` / `%s`.\n',source.run_eto_sha256,source.run_ieo_sha256);
...
function source=verify_executed_sources(code_dir,config)
paths=struct('main',fullfile(code_dir,'run_exp3_fixedC_longruninit_minfix.m'), ...
    'run_eto',fullfile(code_dir,'run_eto.m'), ...
    'run_ieo',fullfile(code_dir,'run_ieo.m'), ...
    'generate_data',fullfile(code_dir,'generate_data.m'));
f=fieldnames(paths); for k=1:numel(f), assert(exist(paths.(f{k}),'file')==2,'Missing formal source.'); end
assert(config.T==50 && config.n==10 && config.Ntr==60 && config.Nev==1000);
assert(config.base_seed==42 && config.seed_offset==20000 && config.lambda==5e-4);
assert(config.omega==1e-6 && config.alpha==0.05 && config.beta==0.90);
assert(config.volatility_floor==0.005 && ...
    abs(config.initial_marginal_variance-2e-5)<1e-15);
assert(config.fixed_C_source_replication==500 && ...
    config.correlation_eigenvalue_floor==3e-5);
assert(config.bootstrap_repetitions==2000 && config.bootstrap_seed==730042);
assert(config.reporting_bootstrap_repetitions==2000 && ...
    config.reporting_bootstrap_seed==20260913);
source=struct();
for k=1:numel(f)
    source.([f{k} '_path'])=fullfile('code',[f{k} '.m']);
    if strcmp(f{k},'main')
        source.main_path=fullfile('code','run_exp3_fixedC_longruninit_minfix.m');
    end
    source.([f{k} '_sha256'])=sha256_file(paths.(f{k}));
end
end
...
if strcmp(c.mode,'dry')
    status(strcmp(names,'S'))="dry_run_10";
elseif c.S~=2000
    status(strcmp(names,'S'))=string(c.simulation_mode);
end
source_file=repmat(string(source.main_path),numel(names),1);
...
fprintf(fid,'# Experiment 3 formal population-regret run failure\n\n');
fprintf(fid,'- Mode: %s\n- Identifier: `%s`\n- Message: %s\n- Time: %s\n\n', ...
    mode,ME.identifier,ME.message,char(datetime('now','Format','yyyy-MM-dd HH:mm:ss Z')));
fprintf(fid,'No figures from this failed invocation should be used.\n\n## Stack\n\n');
for k=1:numel(ME.stack), fprintf(fid,'- `%s` line %d\n',ME.stack(k).name,ME.stack(k).line); end
end

function remove_formal_figures(figures_dir)
...
function rel=report_path(path_value)
[folder,name,ext]=fileparts(char(path_value));
[~,parent]=fileparts(folder);
rel=strrep(fullfile(parent,[name ext]),filesep,'/');
end
...
for k=1:numel(paths)
    assert(exist(paths{k},'file')~=2,'Exp3Formal:ProtectedOutputExists', ...
        'Refusing to overwrite existing formal output: %s',paths{k});
end
end
...
assert(status==0,'Exp3Formal:HashFailure','Could not hash %s',path);
