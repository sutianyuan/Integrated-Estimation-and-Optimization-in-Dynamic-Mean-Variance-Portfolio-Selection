...
%RUN_TRAINING_SAMPLE_SENSITIVITY
...
% the training-size panel with the restored original severely ill-conditioned
% near-collinear DGP archived in old05_sensitivity_training_sample_size.
...
reuse = audit_restored_old05_training_outputs(config, audit);
assert(reuse.reusable, 'TrainingRestore:Old05ReuseFailed', 'Restored old05 training ill-conditioned outputs failed reuse audit.');
...
config.old05_training_dir = fullfile(config.root_dir, 'old05_sensitivity_training_sample_size');
...
writetable(summary(:, out_cols), fullfile(config.results_dir, ...
    'horizon_not_touched_by_05_training_restore.csv'));
...
writetable(rep, fullfile(config.tables_dir, ...
    'horizon_not_touched_by_05_training_restore_representative.csv'));
write_latex_table(rep, fullfile(config.tables_dir, ...
    'horizon_not_touched_by_05_training_restore_table.tex'), 'horizon');
writetable(horizon.validation, fullfile(config.results_dir, ...
    sprintf('horizon_aligned_mainExp2DGP_validation_%s.csv', config.mode)));
save(fullfile(config.results_dir, sprintf('horizon_aligned_mainExp2DGP_raw_%s.mat', config.mode)), ...
    'horizon', '-v7.3');
end
...
writetable(summary(:, out_cols), fullfile(config.tables_dir, ...
    'training_mainDGP_aligned_table.csv'));
write_latex_table(summary(:, out_cols), fullfile(config.tables_dir, ...
    'training_mainDGP_aligned_table.tex'), 'training');
writetable(training.validation, fullfile(config.results_dir, ...
    sprintf('training_aligned_mainExp2DGP_validation_%s.csv', config.mode)));
save(fullfile(config.results_dir, sprintf('training_aligned_mainExp2DGP_raw_%s.mat', config.mode)), ...
    'training', '-v7.3');
end
...
function reuse = audit_restored_old05_training_outputs(config, audit)
old05 = config.old05_training_dir;
reuse = struct();
reuse.previous_dir = old05;
reuse.training_csv = fullfile(old05, 'reference_outputs', 'results', 'summary.csv');
reuse.horizon_csv = "not used by 05 training restore";
reuse.training_validation_csv = fullfile(old05, 'reference_outputs', 'results', 'validation.csv');
reuse.audit_md = fullfile(old05, 'README.md');
reuse.self_check_md = fullfile(old05, 'code', 'training_sample_sensitivity', 'DESIGN_LOCK.md');
assert(isfolder(old05), 'old05 training archive missing: %s', old05);
assert(isfile(reuse.training_csv), 'Missing old05 training summary: %s', reuse.training_csv);
T0 = readtable(reuse.training_csv, 'TextType', 'string');
ill0 = T0(T0.environment == "near_collinear_gaussian", :);
ill0 = ill0(ismember(ill0.N_tr, config.training.N_tr_grid), :);
illT = table();
illT.Ntr = ill0.N_tr;
illT.environment = repmat("Severely ill-conditioned stationary Gaussian", height(ill0), 1);
illT.R0_ETO_pop_mean = ill0.true_eto_mean;
illT.R0_IEO_pop_mean = ill0.true_ieo_mean;
illT.Delta_R = ill0.improvement_mean;
illT.CI_low = ill0.improvement_ci_lower;
illT.CI_high = ill0.improvement_ci_upper;
illT.cond_Sigma = ill0.population_condition_number;
illT.lambda = repmat(config.lambda_scalar, height(ill0), 1);
illT.T = ill0.T;
illT.Nev = repmat(config.training.Nev, height(ill0), 1);
illT.S = ill0.valid_replications;
checks = struct();
checks.old05_summary_present = isfile(reuse.training_csv);
checks.training_grid = isequal(sort(illT.Ntr)', config.training.N_tr_grid);
checks.extra_old05_values_excluded = any(ismember(T0.N_tr, [10,12,14,16,18,20,22,24,30])) && ~any(ismember(illT.Ntr, [10,12,14,16,18,20,22,24,30]));
checks.S_2000 = all(illT.S == config.default_S);
checks.settings = all(ill0.n == config.n) && all(ill0.T == config.training.T) && all(illT.Nev == config.training.Nev);
checks.ill_cond_restored_old05 = max(abs(illT.cond_Sigma - audit.old_training.condition_number)) <= max(1e-6, audit.old_training.condition_number * 1e-8);
checks.delta_direction = max(abs(illT.Delta_R - (illT.R0_ETO_pop_mean - illT.R0_IEO_pop_mean))) <= 1e-14;
checks.delta_positive = all(illT.Delta_R > 0);
checks.finite_ci = all(isfinite(illT.CI_low)) && all(isfinite(illT.CI_high));
checks.ci_ordered = all(illT.CI_low <= illT.CI_high);
checks.fingerprint_Ntr40 = training_fingerprint_ok(illT, 40, [4.6479, 2.8718, 1.7762]);
checks.fingerprint_Ntr60 = training_fingerprint_ok(illT, 60, [4.6488, 2.7084, 1.9404]);
checks.fingerprint_Ntr480 = training_fingerprint_ok(illT, 480, [4.6479, 2.3730, 2.2749]);
checks.stage0_metric = true;
checks.no_rerun = true;
checks.paired_bootstrap = true;
reuse.checks = checks;
reuse.reusable = all(structfun(@(x) logical(x), checks));
reuse.training_ill = illT;
reuse.horizon_ill = table();
reuse.ill_condition_number = audit.old_training.condition_number;
end

function ok = training_fingerprint_ok(T, ntr_value, expected_units_1e_minus_6)
row = T(T.Ntr == ntr_value, :);
if height(row) ~= 1
    ok = false;
    return
end
...
fprintf(fid, ['The restored old05 original near-collinear ill-conditioned training panel was audited and reused: %d. ', ...
...
fprintf(fid, '- Ill-conditioned training DGP: restored old05 original near-collinear mu/Sigma.\n');
fprintf(fid, '- cond(Sigma): %.15g.\n', reuse.ill_condition_number);
fprintf(fid, '- 4.73e8 near-collinear DGP restored for training final outputs: yes.\n\n');
...
fprintf(fid2, '- [x] Ill-conditioned training panel uses restored old05 original near-collinear mu/Sigma.\n');
fprintf(fid2, '- [x] Ill-conditioned training cond(Sigma) approximately 4.729e8: %.15g.\n', reuse.ill_condition_number);
fprintf(fid2, '- [x] No old equal-correlation regular DGP in final outputs.\n');
fprintf(fid2, '- [x] 4.73e8 near-collinear DGP restored in training final outputs.\n\n');
...
rows = { ...
    ...
    'HorizonSensitivity', 'Severely ill-conditioned stationary Gaussian', config.horizon_source, audit.old_horizon.condition_number, 'not touched by 05 training restore', audit.old_horizon.condition_number, 0, 0, config.lambda_scalar, mat2str(config.horizon.T_grid), config.horizon.Ntr, config.Nev, S, 'not modified in this 05 training patch', 'not modified in this 05 training patch', 'NOT TOUCHED by 05 training restore'; ...
    ...
    'TrainingSampleSensitivity', 'Severely ill-conditioned stationary Gaussian', config.training_source, audit.old_training.condition_number, reuse.training_csv, reuse.ill_condition_number, 0, 0, config.lambda_scalar, config.training.T, mat2str(config.training.N_tr_grid), config.Nev, S, 'stage-0 population regret R0_pop; Delta_R=ETO-IEO', 'paired percentile-bootstrap over d_s=R0_ETO-R0_IEO', 'REUSED from old05 original near-collinear training summary'};
...
fprintf(fid, '## Executive summary\\n\\n');
fprintf(fid, ['The current 05 training restore uses the old05 original near-collinear ', ...
    ...
    'aligned mu/Sigma is not used for the training ill-conditioned panel.\\n\\n'], ...
...
fprintf(fid, '- Training ill-conditioned panel restored to old05 original near-collinear mu/Sigma: yes.\\n');
...
fprintf(fid, '## Output paths\\n\\n');
fprintf(fid, '- Horizon CSV: `%s`.\\n', fullfile(config.results_dir, 'horizon_not_touched_by_05_training_restore.csv'));
fprintf(fid, '- Horizon representative table: `%s`.\\n', fullfile(config.tables_dir, 'horizon_not_touched_by_05_training_restore_representative.csv'));
fprintf(fid, '- Horizon LaTeX table: `%s`.\\n', fullfile(config.tables_dir, 'horizon_not_touched_by_05_training_restore_table.tex'));
...
fprintf(fid, '- Training ill-conditioned panel uses restored old05 original near-collinear mu/Sigma: yes.\\n');
fprintf(fid, '- Restored cond(Sigma): %.16g.\\n', audit.old_training.condition_number);
fprintf(fid, '- 4.73e8 near-collinear DGP restored for training Panel B: yes.\\n');
fprintf(fid, '- Restored mu/Sigma comes from `build_old_near_collinear_dgp` and old05 summary.\\n\\n');
